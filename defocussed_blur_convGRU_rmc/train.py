...
IMAGE_DIR_PATH              = "../"
SAVE_PATH            = "./model/deblur_myfcn_working_focussed_"
...
def test(loader, agent, fout, episode):
    ...
    try:
        os.mkdir(f"resultimage_defocussed/{episode}")
    except:
        pass
    try:
        os.mkdir(f"resultimage_defocussed/input")
    except:
        pass
...
#             raw_n[j, 0] = cv2.blur(raw_x[j, 0]*255, ksize = (10, 10)) 
        ...
        cv2.imwrite('./resultimage_defocussed/input/' + str(i) + '_input.png', N)
        cv2.imwrite('./resultimage_defocussed/' + str(episode) + '/' + str(i) + '_output.png',p)
